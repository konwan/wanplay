...
class DataCheck(object) :
    ...
    def dealdetail(self, filedate) :
        ...
        if self.now_dir == "header" :
            ...
            for i in sorted(self.data_dict_date.keys()) :
                if len(self.data_dict_date[i]) < 2 and "-999" not in self.data_dict_date[i] :
                    self.erroroutput("data", self.now_filename, "{}{}{}".format(" [", i, "] only have noncustomer data")) 
                    self.error_msg_mongo["detail"]["noncustomer"].append(i)
                    self.score += -1

        if filedate not in self.data_dict_date.keys() :
            self.erroroutput("data", self.now_filename, "{}{}{}".format(" no [", filedate, "] data"))
            self.error_msg_mongo["detail"]["nofiledate"] = "{}{}{}".format(" no [", filedate, "] data ")
            self.score += -1
        ...
        if self.datatype == "incremental" :
            if len(self.data_dict_date.keys()) > 1 :
                self.erroroutput("data", self.now_filename, "{}{}{}".format(" not only [", filedate, "] data"))
                self.error_msg_mongo["detail"]["notonlyfiledate"] = "{}{}{}".format(" not only [", filedate, "] data ")
                self.score += -1
            if self.dealfirstday != self.deallastday :
                self.erroroutput("info", self.now_filename, "{}{}{}{}{}".format(" deal date is between [ ", self.dealfirstday, " ~ ", self.deallastday, " ]"))
                self.error_msg_mongo["detail"]["filedatenotmatch"] = "{}{}{}{}".format(" deal date is between ", self.dealfirstday, " ~ ", self.deallastday )
        else : 
            losedays = self.dealdatelose()
            losedayscnt = len(losedays)
            if losedayscnt != 0 :
                self.erroroutput("final", self.now_filename, "{}{}{}\n{}".format(" lose ", str(losedayscnt), " days data ====> ", str(losedays)))
    ...
    def dataspec(self, standcolcnt, colcnt) :
        if standcolcnt != colcnt :
            ...
            if self.data_error_cnt <= self.error_print_cnt :
                self.erroroutput("data", self.now_filename, "{}{}{}{}".format(" wrong schema != ", str(colcnt), " ==> ", str(self.tmpline).decode('string_escape')))
            ...
            if self.data_error_empty <= self.error_print_cnt :
                self.erroroutput("data", self.now_filename, "{}{}".format(" has empty column ==> ", str(self.tmpline).decode('string_escape')))
            ...
            for i in self.exclude_char :
                ...
                if i in strline :
                    ...
                    if self.data_error_special_char <= self.error_print_cnt :
                        self.erroroutput("data", self.now_filename, "{}{}{}{}".format(" has spacial char [", i, "] ==> ", str(self.tmpline).decode('string_escape')))
    ...
    def datetimespec(self, dateidx) :
        if self.tmpline[dateidx].find("T") < 0 :
            ...
            if self.date_error_t <= self.error_print_cnt :
                self.erroroutput("data", self.now_filename, "{}{}".format(" wrong datetime format ==> ", str(self.tmpline).decode('string_escape')))
            ...
            if self.date_error_len <= self.error_print_cnt :
                self.erroroutput("data", self.now_filename, "{}{}".format(" wrong datetime length ==> ", str(self.tmpline).decode('string_escape')))
                self.error_msg_mongo["format"]["date"].append(self.tmpline)
            self.score += -1
        else :
    ...
    def ftpdataformat(self, filepath, filelist) :
        for i in filelist :
            ...
            for line in f :
                ...
                if self.now_dir == self.ftp_src_dir[4] : #member
                    if colcnt != 14 :
                        self.data_error_cnt += 1 
                        if self.data_error_cnt <= self.error_print_cnt :
                            self.erroroutput("data", self.now_filename, "{}{}".format(" wrong schema != 14 ==> ", str(self.tmpline).decode('string_escape')))
                            self.error_msg_mongo["format"]["schema"].append(self.tmpline)
                        self.score += -1
                    elif self.tmpline[0] == "" :
                        self.data_error_empty += 1
                        if self.data_error_empty <= self.error_print_cnt :
                            self.erroroutput("data", self.now_filename, "{}{}".format(" empty member id(1) ==> ", str(self.tmpline).decode('string_escape')))
                            self.error_msg_mongo["format"]["empty"].append(self.tmpline)
                        self.score += -1
                    elif self.tmpline[0].find("-") > -1 :
                        self.data_error_special_char += 1
                        if self.data_error_special_char <= self.error_print_cnt :
                            self.erroroutput("data", self.now_filename, "{}{}".format(" couldn't use ['-'] ==> ", str(self.tmpline).decode('string_escape')))
                            self.error_msg_mongo["format"]["specialchar"].append(self.tmpline)
                        self.score += -1 
                    ...
                    if colcnt != 7 :
                        self.data_error_cnt += 1
                        if self.data_error_cnt <= self.error_print_cnt :
                            self.erroroutput("data", self.now_filename, "{}{}".format(" wrong schema != 7 ==> ", str(self.tmpline).decode('string_escape')))
                            self.error_msg_mongo["format"]["schema"].append(self.tmpline) 
                        self.score += -1
                    elif "" in [self.tmpline[1], self.tmpline[2], self.tmpline[5], self.tmpline[6]] :
                        self.data_error_empty += 1
                        if self.data_error_empty <= self.error_print_cnt :
                            self.erroroutput("data", self.now_filename, "{}{}".format(" trx_id(2), item_id(3), money(6), date(7) shouldn't be empty ==> ", str(self.tmpline).decode('string_escape')))
                            self.error_msg_mongo["format"]["empty"].append(self.tmpline)
                        self.score += -1
                    else :
                        if self.checkdealdate :
            ...
            if self.score < 0 :
                self.error_msg += "-------------------------------------------- [{}] ==> File [data - error] Score {}------------------------------------------------\n\n".format(self.now_filename, str(self.score))
    ...
    def ftpdatamark(self, data_list, data_mark_list, replacestr, namesep, mark, filepath) :
        for i in data_list :
            ...
            if (src_file_name + mark) not in data_mark_list :
                self.erroroutput("ftp", filepath, "{} {} {}{}{}{}".format(":", line[0], self.datatype, " data no mark [", mark, "] file"))
                self.error_msg_dir_mongo["overall"]["mark"][self.datatype].append("{} {}{}{}{}".format(line[0], self.datatype, " data no mark [", mark, "] file "))
                self.dir_score += -1
    ...
    def ftpfilerule(self, ftp_src_path) :
        ...
        if not os.path.exists(ftp_src_path) or os.stat(ftp_src_path).st_size == 0 :
            self.erroroutput("ftp", ftp_src_path, " is not exist")
            self.error_msg_dir_mongo["overall"]["path"]["dir"] = "{}{}".format(self.now_dir, " is not exist ")
            self.dir_score += -1
        else :
            ftp_src_file = os.listdir(ftp_src_path)
            if len(ftp_src_file) == 0 :
                self.erroroutput("ftp", ftp_src_path, " no data")
                ...
                if self.checkInc :
                    ...
                    if len(src_Inc) > 0 :
                        ...
                        self.erroroutput("ftp", ftp_src_path, " no incremental data")
                ...
                if self.checkHis :
                    ...
                    if len(src_His) > 0 :
                        ...
                        self.erroroutput("ftp", ftp_src_path, " no historical data")
                        self.error_msg_dir_mongo["overall"]["mark"]["historical"].append(" no historical data ")
                        self.dir_score += -1
    
    def erroroutput(self, prefix, path, msg) :
        self.error_msg += "[{} - {}] - {} {}\n".format(prefix, self.now_dir, path, msg)
    ...
    if checkftp :
        for i in ftp_src_dir :
            ...
            if dc.checkHis and dc.checkInc :
                dc.final_msg += "[final - {}] - {} total score for {}\n".format(dc.now_dir, ftp_src_path, str(dc.dir_score))
            elif dc.checkHis :
                dc.final_msg += "[final - {}] - {} total score for [ historical data ] {}\n".format(dc.now_dir, ftp_src_path, str(dc.dir_score))
            else :
                dc.final_msg += "[final - {}] - {} total score for [ incremental data ] {}\n".format(dc.now_dir, ftp_src_path, str(dc.dir_score))

            writefile(("/root/tmp/cindy/" + dc.shop_id + ".txt"), dc.error_msg)
            insertjson = {
            "TaskID" : dc.task_name,
            ...
            for val in dc.overalldata.keys():
                ...
                if losedays > 0 :
                    losedaystr = " losdays for {} days => {}".format(str(len(dc.overalldata[val]["losedays"])), str(list(dc.overalldata[val]["losedays"])))
                dc.final_msg += "[final - {}] - {} date between [ {} ~ {} ] {}\n".format(dc.now_dir, val, min(dc.overalldata[val]["daterange"]), max(dc.overalldata[val]["daterange"]), losedaystr)
            if dc.now_dir == "header" :
                for val in dc.customer_deal_type.keys():
                    dc.final_msg += "[final - {}] - {} {} % customer data \n".format(dc.now_dir, val, str(round((float(dc.customer_deal_type[val][0]) / float(dc.customer_deal_type[val][0] + dc.customer_deal_type[val][1])) * 100 ,2 )))

        writefile(("/root/tmp/cindy/overall.txt"), "{}\n{}\n\n".format(dc.shop_id + "-------------------",dc.final_msg)) 
        print dc.final_msg

    if checkhadoop :
        dc.error_msg = ""
        one_time_result = dc.hadoop_path + dc.shop_id
